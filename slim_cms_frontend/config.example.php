...
if(isset($_SERVER['SERVER_ADDR']) && ($_SERVER['SERVER_ADDR'] == '127.0.0.1') || $_SERVER['SERVER_ADDR'] == '192.168.1.56') {
...
function GreenBizCheckLoader($class) {
	if(is_file(PATH_LIB.'/'.str_replace('\\', '/',$class) .'.class.php')) {
		require_once(PATH_LIB.'/'.str_replace('\\','/',$class) .'.class.php');
	}
}

function pluginLoader($class) {
	if(is_file(PATH_PLUGIN.'/'.$class.'/'.$class.'.class.php')) {
...
spl_autoload_register('GreenBizCheckLoader');
