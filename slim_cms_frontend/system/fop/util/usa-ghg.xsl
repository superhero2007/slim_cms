  ...
  <!-- //End of metrics list -->
	...
	<xsl:template match="averageGHGResult" mode="xhtml">
		...
		<fo:block-container background-color="rgb(221,234,251)" padding-top="5mm" padding-bottom="5mm" color="rgb(0,44,93)" margin-bottom="5mm">
			...
			<fo:block margin-bottom="5mm">The average result is <xsl:value-of select="format-number($averageResult,'###,##0.000')" /> Tons CO2-e per square foot of floor space. Your result is <xsl:value-of select="format-number($ghgFloorSpaceResult, '###,##0.000')" /> Tons CO2-e per square foot of floor space.</fo:block>
