...
<xsl:stylesheet version="1.0"
	...
	<xsl:template match="combinedGHGBarGraph" mode="xhtml">
		...
		<xsl:variable name="graph_url">
			<xsl:text>http://www.greenbizcheck.com/graph/?g=barGraph&amp;w=1000&amp;h=500&amp;graphTitle=Total-Emmissions-(Tonnes)&amp;unique_time_stamp=</xsl:text>
	...
	<xsl:template match="combinedBarGraph" mode="xhtml">
		...
		<xsl:variable name="graph_url">
			<xsl:text>http://www.greenbizcheck.com/graph/?g=barGraph&amp;w=1000&amp;h=500&amp;graphTitle=Total-Greenhouse-Gas-Output-(Tonnes)&amp;unique_time_stamp=</xsl:text>
			<xsl:value-of select="$unique_time_stamp" />
			<xsl:text>-combinedScopeBarGraph</xsl:text>
			<xsl:for-each select="/config/plugin[@plugin = 'checklist']/report">
	...
	<xsl:template match="scope1BarGraph" mode="xhtml">
		...
		<!-- //Build the URL for the graph image -->
		<xsl:variable name="graph_url">
			<xsl:text>http://www.greenbizcheck.com/graph/?g=barGraph&amp;w=1000&amp;h=500&amp;graphTitle=Scope-1-Greenhouse-Gas-Output-(Tonnes)&amp;unique_time_stamp=</xsl:text>
			<xsl:value-of select="$unique_time_stamp" />
	...
	<xsl:template match="scope1GHGBarGraph" mode="xhtml">
		...
		<!-- //Build the URL for the graph image -->
		<xsl:variable name="graph_url">
			<xsl:text>http://www.greenbizcheck.com/graph/?g=barGraph&amp;w=1000&amp;h=500&amp;graphTitle=Fuel-Based-Emissions-(Tonnes)&amp;unique_time_stamp=</xsl:text>
			<xsl:value-of select="$unique_time_stamp" />
			<xsl:text>-scope1BarGraph</xsl:text>
			<xsl:for-each select="/config/plugin[@plugin = 'checklist']/report">
				<xsl:sort select="questionAnswer/answer[@answer_type = 'date-month']/@arbitrary_value" />
	...
	<xsl:template match="scope2BarGraph" mode="xhtml">
		<!-- //Build the URL for the graph image -->
		<xsl:variable name="graph_url">
			<xsl:text>http://www.greenbizcheck.com/graph/?g=barGraph&amp;w=1000&amp;h=500&amp;graphTitle=Scope-2-Greenhouse-Gas-Output-(Tonnes)&amp;unique_time_stamp=</xsl:text>
			<xsl:value-of select="$unique_time_stamp" />
	...
	<xsl:template match="scope2GHGBarGraph" mode="xhtml">
		...
		<!-- //Build the URL for the graph image -->
		<xsl:variable name="graph_url">
			<xsl:text>http://www.greenbizcheck.com/graph/?g=barGraph&amp;w=1000&amp;h=500&amp;graphTitle=Electricity-Based-Emissions-(Tonnes)&amp;unique_time_stamp=</xsl:text>
			<xsl:value-of select="$unique_time_stamp" />
			<xsl:text>-scope2BarGraph</xsl:text>
			<xsl:for-each select="/config/plugin[@plugin = 'checklist']/report">
				<xsl:sort select="questionAnswer/answer[@answer_type = 'date-month']/@arbitrary_value" />
    ...
    <xsl:template match="scope3BarGraph" mode="xhtml">
		<!-- //Build the URL for the graph image -->
		<xsl:variable name="graph_url">
			<xsl:text>http://www.greenbizcheck.com/graph/?g=barGraph&amp;w=1000&amp;h=500&amp;graphTitle=Scope-3-Greenhouse-Gas-Output-(Tonnes)&amp;unique_time_stamp=</xsl:text>
			<xsl:value-of select="$unique_time_stamp" />
	...
	<xsl:template match="electricityBarGraph" mode="xhtml">
		...
		<!-- //Build the URL for the graph image -->
		<xsl:variable name="graph_url">
			<xsl:text>http://www.greenbizcheck.com/graph/?g=barGraph&amp;w=1000&amp;h=500&amp;graphTitle=Electricity-Cost-per-m2(in-$)&amp;unique_time_stamp=</xsl:text>
			<xsl:value-of select="$unique_time_stamp" />
	...
	<xsl:template match="electricityKwhBarGraph" mode="xhtml">
		...
		<!-- //Build the URL for the graph image -->
		<xsl:variable name="graph_url">
			<xsl:text>http://www.greenbizcheck.com/graph/?g=barGraph&amp;w=1000&amp;h=500&amp;graphTitle=Electricity-Consumption-per-m2(kWh)&amp;unique_time_stamp=</xsl:text>
			<xsl:value-of select="$unique_time_stamp" />
	...
	<xsl:template match="averageIndustryElectricityConsumption" mode="xhtml">
		...
		<fo:block margin-bottom="10mm">
			The industry average consumption of electricity is <xsl:value-of select="format-number(($average-annual-electricity-consumption div $average-annual-store-size), '###,##0.00')" />kWh/m<fo:inline vertical-align="super" font-size="8pt">2</fo:inline> per annum.
		</fo:block>
	...
	<xsl:template match="radarGraphCategoryScores" mode="xhtml">
     ...
     <xsl:template match="googleImageChart" mode="xhtml">
         <xsl:variable name="graph_url">
            ...
            <xsl:text>&amp;chdl=Tonnes</xsl:text>				<!-- //Data Label -->
	...
	<xsl:template match="lastGHGEntryPieGraph" mode="xhtml">
		<xsl:for-each select="/config/plugin[@plugin = 'checklist']/report">
        	<xsl:if test="position() = last()"> 
            	...
            	<!-- //Build the URL for the graph image -->
                <xsl:variable name="graph_url">
                    <xsl:text>http://www.greenbizcheck.com/graph/?g=pie3dGraph&amp;w=1000&amp;h=500&amp;graphTitle=Total-Greenhouse-Gas-Output-(Tonnes)&amp;unique_time_stamp=</xsl:text>
                    <xsl:value-of select="$unique_time_stamp" />
                    ...
                    <xsl:for-each select="metricGroup">
                        ...
                        <xsl:text>&amp;labels[]=</xsl:text><xsl:value-of select="translate(concat(sum(clientMetric[@ghg_calculation != 'ignore']/@ghg_calculation), ' Tonnes'), ' ','-')" />
                    </xsl:for-each>
                </xsl:variable>
	...
	<xsl:template match="lastGHGEntryPieGraphItems" mode="xhtml">
		<xsl:for-each select="/config/plugin[@plugin = 'checklist']/report">
        	<xsl:if test="position() = last()"> 
            	...
            	<!-- //Build the URL for the graph image -->
                <xsl:variable name="graph_url">
                    <xsl:text>http://www.greenbizcheck.com/graph/?g=pieGraph&amp;w=950&amp;h=750&amp;graphTitle=Total-Greenhouse-Gas-Output-(Tonnes)&amp;unique_time_stamp=</xsl:text>
                    <xsl:value-of select="$unique_time_stamp" />
                    <xsl:text>-combinedScopePieGraph</xsl:text>
                    ...
                    <xsl:for-each select="metricGroup/clientMetric[@ghg_calculation != 'ignore']">
                        ...
                        <xsl:text>&amp;labels[]=</xsl:text><xsl:value-of select="translate(concat(@ghg_calculation, ' Tonnes'), ' ','-')" />
                    </xsl:for-each>
                </xsl:variable>
