...
<!-- ========================= ROOT TEMPLATE =========================== -->
	...
	<xsl:template match="/">
		...
		<xsl:variable name="cover_color">
			<xsl:choose>
				<xsl:when test="$report/@report_cover_color != ''">
					<xsl:value-of select="$report/@report_cover_color" />
				</xsl:when>
				<xsl:otherwise>
		...
		<fo:root>
			<fo:layout-master-set>
				...
				<fo:simple-page-master master-name="cover-page" page-height="297mm" page-width="210mm" margin="0mm">
					<fo:region-body padding="0%" region-name="xsl-region-body" background-color="{$cover_color}" />
			...
			<fo:page-sequence master-reference="cover-page">
				<fo:flow flow-name="xsl-region-body" font-family="Arial">
					...
					<fo:block-container position="absolute" top="40mm">
						<fo:block text-align="center" color="#ffffff" font-size="38pt" margin="10mm" font-weight="bold" line-height="38pt">
							<xsl:choose>
					...
					<!-- //Cover Page Image -->
					<xsl:if test="$logo != ''">
						<fo:block-container position="absolute" top="40%" width="100%" text-align="center">
							<fo:block>
								<fo:external-graphic src="url({$logo})" scaling="non-uniform" content-width="33%" />
							</fo:block>
						</fo:block-container>
					</xsl:if>
					...
					<fo:block-container position="absolute" top="275mm">
						<fo:block background-color="#ffffff" padding-left="16px" padding-right="0" padding-top="15px" padding-bottom="25px" margin-left="0px" margin-right="0px">
							<fo:block-container> 				
								<fo:block font-size="16px" font-weight="bold" color="#333"><xsl:value-of select="$companyName"/></fo:block>
							</fo:block-container> 
